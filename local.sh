...
# github: https://github.com/ShmilyStar/build-actions
#====================================================
...
rm -rf chuang && git clone https://github.com/ShmilyStar/build-actions chuang > /dev/null 2>&1
if [[ ! -d "chuang/build" ]]; then
  rm -rf chuang && svn co https://github.com/ShmilyStar/build-actions/trunk/build chuang/build > /dev/null 2>&1
...
cd ${GITHUB_WORKSPACE}
wget -q https://raw.githubusercontent.com/ShmilyStar/common/main/common.sh -O common.sh
if [[ $? -ne 0 ]]; then
  curl -fsSL https://raw.githubusercontent.com/ShmilyStar/common/main/common.sh -o common.sh
fi
...
cd ${GITHUB_WORKSPACE}
bash -c  "$(curl -fsSL https://raw.githubusercontent.com/ShmilyStar/common/main/custom/first.sh)"
...
source ${GITHUB_WORKSPACE}/operates/${FOLDER_NAME}/settings.ini
wget -q https://raw.githubusercontent.com/ShmilyStar/common/main/common.sh -O ${GITHUB_WORKSPACE}/common.sh
if [[ $? -ne 0 ]]; then
  curl -fsSL https://raw.githubusercontent.com/ShmilyStar/common/main/common.sh -o ${GITHUB_WORKSPACE}/common.sh
fi
...
if [[ `echo "${PATH}" |grep -ic "windows"` -ge '1' ]] && [[ ! "${WSL_ROUTEPATH}" == 'true' ]]; then
  ...
  case ${Bendi_Wsl} in
  [Yy])
    bash -c  "$(curl -fsSL https://raw.githubusercontent.com/ShmilyStar/bendi/main/wsl.sh)"
    exit 0
  ;;
  *)
...
ECHOGG "更新扩展文件"
git clone -b main --depth 1 https://github.com/ShmilyStar/common ${HOME_PATH}/build/common
judge "更新扩展文件"
...
function Bendi_Packaging() {
  ...
  curl -fsSL https://github.com/ShmilyStar/common/releases/download/API/${kernel_usage}.api -o amlogic/${kernel_usage}.api
  if [[ $? -ne 0 ]]; then
    curl -fsSL https://github.com/ShmilyStar/common/releases/download/API/${kernel_usage}.api -o amlogic/${kernel_usage}.api
  fi
